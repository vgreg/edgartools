...
from edgar.company_reports import EightK, PressRelease, TenK, TenQ, TwentyF
...
def test_tenk_filing_with_no_gaap():
    ...
    filing = Filing(form="10-K", filing_date="2023-04-06", company="Frontier Masters Fund", cik=1450722, accession_no="0001213900-23-028058")
...
def test_tenq_filing():
    filing = Filing(form="10-Q", filing_date="2023-04-06", company="NIKE, Inc.", cik=320187, accession_no="0000320187-23-000013")
...
def test_items_for_8k_filing():
    filing = Filing(form="8-K", filing_date="2023-11-14", company="ALPINE 4 HOLDINGS, INC.", cik=1606698, accession_no="0001628280-23-039016")
...
def test_eightk_with_spaces_in_items():
    ...
    filing = Filing(form="8-K", filing_date="2023-03-20", company="AAR CORP", cik=1750, accession_no="0001104659-23-034265")
    eightk = filing.obj()
    print()
    print(eightk)
...
def test_eightk_with_no_signature_header():
    ...
    filing = Filing(form="8-K", filing_date="2023-03-20", company="AMERISERV FINANCIAL INC /PA/", cik=707605, accession_no="0001104659-23-034205")
    eightk = filing.obj()
...
def test_eightk_difficult_parsing():
    ...
    filing = Filing(form="8-K", filing_date="2023-03-20", company="4Front Ventures Corp.", cik=1783875, accession_no="0001279569-23-000330")
    eightk = filing.obj()
    assert eightk.items == ["Item 5.02"]
    assert "appointed Kristopher Krane as a member of the Board" in eightk["Item 5.02"]

    filing = Filing(form="8-K", filing_date="2023-03-20", company="ALBEMARLE CORP", cik=915913, accession_no="0000915913-23-000088")
    eightk = filing.obj()
    ...
    filing = Filing(
        ...
        cik=1498148,
        accession_no="0001493152-23-008256",
    )
    eightk = filing.obj()
    assert eightk.items == ["Item 8.01", "Item 9.01"]
...
def test_eightk_with_items_split_by_newlines():
    filing = Filing(form="8-K", filing_date="2023-03-20", company="AFC Gamma, Inc.", cik=1822523, accession_no="0001829126-23-002149")
...
def test_create_eightk_obj_and_find_items():
    adobe_8K = Filing(form="8-K", filing_date="2023-03-15", company="ADOBE INC.", cik=796343, accession_no="0000796343-23-000044")
    eightk = adobe_8K.obj()
...
def test_get_press_release():
    ...
    # Exhibit EX-99.1
    filing = Filing(form="8-K", filing_date="2024-03-08", company="3M CO", cik=66740, accession_no="0000066740-24-000023")
    eightk = filing.obj()
    assert eightk.has_press_release
    press_release: PressRelease = eightk.press_release
...
def test_get_press_release_for_8k_multiple_EX99_files():
    filing = Filing(form="8-K", filing_date="2024-02-06", company="UDR, Inc.", cik=74208, accession_no="0000074208-24-000005")
    eightK: EightK = filing.obj()
    assert eightK.has_press_release
    press_release: PressRelease = eightK.press_release
